...
// ####### FEEDBACK SERIAL OUT #######
      ...
      { // Send data periodically every 10 ms
        ...
        Feedback.cmd2 = (int16_t)rtY_Left.iq; //overwrite
        // Feedback.speedR_meas	  = (int16_t)rtY_Right.n_mot;
        Feedback.speedR_meas = (int16_t)left_dc_curr; //overwrite right speed with left_dc_current
      ...
      if ((TEMP_POWEROFF_ENABLE && board_temp_deg_c >= TEMP_POWEROFF && speedAvgAbs < 20) || (batVoltage < BAT_DEAD && speedAvgAbs < 20))
      ...
      else if (BEEPS_BACKWARD && ((speed < -50 && speedAvg < 0) || MultipleTapBrake.b_multipleTap))
